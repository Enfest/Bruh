...
f = open("clinic_child.csv", "r")
wf = open("clinic_child_latlng.csv", "w")
...
for i in range(400):
    ...
    if info[0] == "":
        break
    spec = ",".join(info[3:])
    tt = client.geocode(info[2])
    print(tt[0]['geometry']['location']["lng"])
    wf.write(info[0]+',' + str(tt[0]['geometry']['location']["lat"])+"," +
             str(tt[0]['geometry']['location']["lng"])+','+info[2]+','+spec+"\n")
